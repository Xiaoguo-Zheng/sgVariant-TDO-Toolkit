...
## 🔬 Note on Algorithmic Stringency
This Python implementation prioritizes **data integrity and high-confidence mapping**. 
If you compare the output counts with legacy Perl scripts, you may observe slightly lower total counts. This is expected. The legacy scripts often utilized hardcoded partial barcode matches to forcibly assign low-quality, single-sided reads to samples. This Python pipeline enforces strict dual-primer matching and dynamic dictionary lookups, ensuring that every counted read is a verified, high-quality amplicon.